...
for mod in disallowedMods {
    if(loadedMods has mod) {
        for item in loadedMods[mod].items {
            ...
            ItemStages.setUnfamiliarName(game.localize("greedycraft.stage.disabled_item.name"), item);
...
events.onPlayerTick(function(event as crafttweaker.event.PlayerTickEvent) {
    ...
    if(!player.creative && player.world.getWorldTime() % 40 == 0) {
        ...
        player.addPotionEffect(<potion:minecraft:wither>.makePotionEffect(200, 2, false, false));
    }
    
    if(!player.creative && player.world.getWorldTime() % 1200 == 0) {
